...
import { DbCouplet, prepareDbCouplets } from '@/server/db/mappings/couplet.mapper';
...
export async function sheetToJson(
  sheetName: string
): Promise<{ rawCouplets: SheetData; couplets: DbCouplet[]; tags: DbTag[] }> {
  ...
  try {
    ...
    const rawCouplets = parsedData.data;
    const couplets = prepareDbCouplets(rawCouplets);
    if (couplets.length === 0) {
      throw new ApiError('No valid couplets found in sheet data', 400);
    }

    const tags = prepareDbTags(rawCouplets);

    return { rawCouplets, couplets, tags };
  } catch (error) {
