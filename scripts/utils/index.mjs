...
export const generateShortHash = (text) =>
	crypto.createHash("sha256").update(text).digest("hex").substring(0, 6);
...
export const createSlug = (text, replacement = "-") =>
	slugify(latinize(text), { lower: true, replacement, strict: true, trim: true });
...
export const mapCsvDataToJson = (data) => {
	...
	return jsonData.map((row) =>
		Object.keys(row).reduce(
			(newRow, key) => ({ ...newRow, [columnMapping[key] || key]: row[key] }),
			{}
		)
	);
};
